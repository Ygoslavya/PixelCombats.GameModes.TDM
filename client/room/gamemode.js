...
function SetMockMode(winners, loosers) { 
	...
	// время спавна для проигравших и победителей
	var spawnsLosers= Spawns.GetContext(loosers); 
	spawnsLosers.RespawnTime.value= 2;

	var spawnsWinners= Spawns.GetContext(winners); 
	spawnsWinners.RespawnTime.value= 1;

	var inventoryLosers= Inventory.GetContext(loosers); 
	inventoryLosers.Main.value= false; 

	var inventoryWinners= Inventory.GetContext(winners); 

	inventoryWinners.MainInfinity.value= true; 
